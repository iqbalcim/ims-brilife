...
const statusConfig: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  PAID: { label: 'Lunas', color: 'bg-green-100 text-green-800', icon: <Check className="h-3 w-3" /> },
  PENDING: { label: 'Menunggu', color: 'bg-yellow-100 text-yellow-800', icon: <Clock className="h-3 w-3" /> },
  OVERDUE: { label: 'Jatuh Tempo Terlewat', color: 'bg-orange-100 text-orange-800', icon: <AlertTriangle className="h-3 w-3" /> },
  FAILED: { label: 'Gagal', color: 'bg-red-100 text-red-800', icon: <XCircle className="h-3 w-3" /> },
  REFUNDED: { label: 'Dikembalikan', color: 'bg-purple-100 text-purple-800', icon: <RotateCcw className="h-3 w-3" /> },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Pembayaran Premi</h2>
          <p className="text-muted-foreground">Kelola pembayaran premi polis</p>
        </div>
        <Button asChild>
                  ...
                  {payments.map((payment) => (
                    <tr key={payment.id} className="border-b hover:bg-muted/50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium ${statusConfig[payment.status]?.color}`}>
